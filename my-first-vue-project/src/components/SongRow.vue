...
<template>
    ...
    >
        <div class="flex items-center w-full py-1.5">
            <div v-if="isHover" class="w-[40px] ml-[14px] mr-[6px] cursor-pointer">
                <Play
                    ...
                    fillColor="#FFFFFF"
                    :size="25"
                    @click="useBeat.playOrPauseThisBeat(artist, track)"
                />
                <Play
                    v-else-if="isPlaying && currentTrack.name !== track.name"
                    fillColor="#FFFFFF"
                    :size="25"
                    @click="useBeat.loadBeat(artist, track)"
                />

                <Pause v-else fillColor="#FFFFFF" :size="25" @click="useBeat.playOrPauseBeat()"/>
            </div>
        ...
        <div class="flex items-center">
            <button type="button" class="pr-4" v-if="isHover">
                <Heart fillColor="#1BD760" :size="21"/>
            </button>
            <div
                v-if="isTrackTime"
                class="text-xs mx-8 pr-4 text-gray-400"
            >
                {{ isTrackTime }}
            </div>
        </div>
